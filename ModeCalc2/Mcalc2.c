...
//------- begin reading fundamental parameters -------
	...
	int BoundC[2];
	double omega0;
	input_file0(fp_i0, BoundC, f_prefix, p_cross, InPrecision, Xi_ctrl, &omega0);//revised on 20200714
...
//-------  end reading fundamental parameters  -------
	for(int i1 = 0 ; i1 < 2 ; i1++){
		...
		main_calc(argv, data_name, out_name, BoundC, InPrecision, Xi_name, omega0);//revised on 20200714
	}
	end_1st:;
}
//======================================================================
...
//======================================================================
//  checkM                             Last updated on May 18, 2020.  
//======================================================================
void checkM(const long N, const double complex maa[N*N], const double complex mbb[N*N])
{
	...
	for(long icolumn = 0 ; icolumn < N ; icolumn++){
		for(long irow = 0 ; irow < N ; irow++){
			if(fabs(creal(maa[irow+N*icolumn]) - creal(maa[icolumn+N*irow])) > 100.*DBL_EPSILON){	fprintf(stderr,"transpose error of maa in checkM! %.5e\n",creal(maa[irow+N*icolumn]) - creal(maa[icolumn+N*irow]));}
			if(fabs(creal(mbb[irow+N*icolumn]) - creal(mbb[icolumn+N*irow])) > 100.*DBL_EPSILON){	fprintf(stderr,"transpose error of mbb in checkM! %.5e\n",creal(mbb[irow+N*icolumn]) - creal(mbb[icolumn+N*irow]));}
		}
	}
}
//======================================================================
...
void set_D(const int BoundC[2], const int L[2], double Dup0[], double Dup1[], double Ddown0[], double Ddown1[])
{
	...
	for(long i1 = 0 ; i1 < L[1] ; i1++){
		...
		for(long i0 = 0 ; i0 < L[0] ; i0++){
			...
			long i0a = (i0 + 1)%L[0];
			long i0b = (i0 + 2)%L[0];
			long i0c = (i0 - 1 + L[0])%L[0];
			long ipoint = i0 + L[0]*i1;
			long ipoint_0a = i0a +  L[0]*i1;
			long ipoint_1a = i0  + L[0]*i1a;
			long ipoint_0b = i0b +  L[0]*i1;
			long ipoint_1b = i0  + L[0]*i1b;
			long ipoint_0c = i0c +  L[0]*i1;
			long ipoint_1c = i0  + L[0]*i1c;
			Dup0[ipoint + L[0]*L[1]*ipoint_0c] = (1./24.)*((double) ((1-swm[0]) + swm[0]*BoundC[0]));
			Dup0[ipoint + L[0]*L[1]*ipoint] = -9./8.;
			Dup0[ipoint + L[0]*L[1]*ipoint_0a] = (9./8.)*((double) ((1-sw[0]) + sw[0]*BoundC[0]));
			Dup0[ipoint + L[0]*L[1]*ipoint_0b] = (-1./24.)*((double) ((1-swp[0]) + swp[0]*BoundC[0]));
//
			Dup1[ipoint + L[0]*L[1]*ipoint_1c] = (1./24.)*((double) ((1-swm[1]) + swm[1]*BoundC[1]));
			Dup1[ipoint + L[0]*L[1]*ipoint] = -9./8.;
			Dup1[ipoint + L[0]*L[1]*ipoint_1a] = (9./8.)*((double) ((1-sw[1]) + sw[1]*BoundC[1]));
			Dup1[ipoint + L[0]*L[1]*ipoint_1b] = (-1./24.)*((double) ((1-swp[1]) + swp[1]*BoundC[1]));
